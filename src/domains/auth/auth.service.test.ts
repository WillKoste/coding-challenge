...
describe('auth.service', async () => {
	...
	describe('validateWhitelistedIP', () => {
		it('should return as valid (China)', async () => {
			const result = await validateWhitelistedIP('123.123.123.123', [{country_name: 'China', is_valid: true, id: 1}, {country_name: 'United States', is_valid: false, id: 2}]);
			const expected = undefined
			expect(result).toBe(expected);
		});
		it('should return as valid (United States)', async () => {
			const result = await validateWhitelistedIP('174.143.169.143', [{country_name: 'China', is_valid: false, id: 1}, {country_name: 'United States', is_valid: true, id: 2}]);
			const expected = undefined;
			expect(result).toBe(expected);
		});
		it('should return as invalid (United States)', async () => {
			await expect(validateWhitelistedIP('174.143.169.143', [{country_name: 'China', is_valid: false, id: 1}, {country_name: 'United States', is_valid: false, id: 2}])).rejects.toThrow(ErrorMessage.BAD_REQUEST)
		});
	});
	...
	describe('login', () => {
		...
		it('should throw error if IP is not provided', async () => {
			await expect(() => login({
				...
				ip: undefined,
				password: 'Hello1234!'
			})).rejects.toThrow(`[auth.service.getCountryByIP] Error - ${ErrorMessage.BAD_REQUEST}`)
		});
		it('should throw error if password is incorrect', async () => {
			await expect(() => login({
				...
				ip: '174.143.169.143',
				password: 'Hello12345!'
			})).rejects.toThrow(`[auth.service.validatePassword] Error - ${ErrorMessage.INVALID_CREDENTIALS}`)
		});
		it('should throw error if no password is provided', async () => {
			await expect(() => login({
				...
				ip: '174.143.169.143',
				password: undefined,
			})).rejects.toThrow(`[auth.service.login] Error - ${ErrorMessage.INVALID_CREDENTIALS}`)
		});
	});
});
