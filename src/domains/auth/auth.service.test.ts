...
describe("auth.service", async () => {
  ...
  describe("login", () => {
    it("should succesfully log in user", async () => {
      ...
      /**
       * This should be:
       * const expected = {{id: 2, email: 'willkoste+2@gmail.com',whitelisted_location: [{ id: 1, is_valid: false, country_name: 'China' },
       *   { id: 2, is_valid: true, country_name: 'United States' }
       * ]}}
       *
       * expect(result).toEqual(expected)
       *
       * This ensures that the user's `password` (or any other private values) are not exposed via this endpoint's response
       */
